class Controller {
	...
	init(selectedSong, songData, autoPlayEnabled, multiplayer, touchEnabled) {
		...
		// Dan-i Dojo mode detection
		this.isDan = selectedSong.difficulty === "dan"
		if (this.isDan) {
			this.saveScore = false // Don't save individual scores in Dan mode
			// Initialize Dan-i stats tracking
			this.danStats = {
				good: 0,
				ok: 0,
				bad: 0,
				...
				gauge: 0,
				songIndex: 0,
				totalSongs: (this.parsedSongData.nextSongs || []).length + 1
			}
			// Get exam conditions from parsed data (stored in difficulty metadata)
			var danMeta = this.parsedSongData.metadata["dan"] || {}
			this.danExams = danMeta.exams || []
			this.nextSongs = this.parsedSongData.nextSongs || []
		}
	...
	loadNextDanSong() {
		// Get the next song info from nextSongs array
		var nextSongIndex = this.danStats.songIndex
		if (nextSongIndex >= this.nextSongs.length) {
			// No more songs, this shouldn't happen but safety check
			this.displayResults()
			return
		}

		var nextSong = this.nextSongs[nextSongIndex]
		// nextSong structure: {title, subtitle, wave, scoreinit, scorediff, ms}

		// Stop current music and clean up current game state
		this.stopMainLoop()
		if (this.game.mainAsset) {
			this.game.mainAsset.stop()
		}

		// Find the song in assets by matching the wave file or use the embedded data
		var songFolder = this.selectedSong.folder
		var nextWaveUrl = gameConfig.songs_baseurl + songFolder + "/" + nextSong.wave

		// Show transition screen (TODO: could be improved with animation)
		loader.changePage("game", false)

		// Load the next song's audio
		var promises = []
		var nextAudio = new RemoteFile(nextWaveUrl)
		promises.push(snd.musicGain.load(nextAudio).then(sound => {
			this.game.mainAsset = sound
			this.mainAsset = sound
		}))

		Promise.all(promises).then(() => {
			// Reset game state for next song
			this.game.elapsedTime = 0
			this.game.currentCircle = -1
			...
			this.game.globalScore = {
				...
				maxCombo: 0,
				drumroll: 0,
				gauge: this.danStats.gauge, // Carry over gauge from previous song
				title: nextSong.title || this.selectedSong.title,
				difficulty: this.selectedSong.difficulty
			}
			this.game.paused = false
			this.game.started = false
			this.game.mainMusicPlaying = false
			this.game.musicFadeOut = 0
			this.game.fadeOutStarted = false
			this.game.lastCircle = null

			// Restart the main loop
			this.startMainLoop()
		}).catch(error => {
			console.error("Failed to load next Dan song:", error)
			this.displayResults()
		})
	}
	clean() {

	...
	whenFadeoutMusic() {
		...
		if (started) {
			...
			if (this.musicFadeOut === 0) {
				...
				// Dan-i Dojo: Check if there are more songs to play
				if (this.controller.isDan && this.controller.nextSongs && this.controller.danStats.songIndex < this.controller.nextSongs.length) {
					// Accumulate current song stats
					var score = this.getGlobalScore()
					this.controller.danStats.good += score.good
					this.controller.danStats.ok += score.ok
					this.controller.danStats.bad += score.bad
					this.controller.danStats.drumroll += score.drumroll
					this.controller.danStats.score += score.points
					if (score.maxCombo > this.controller.danStats.maxCombo) {
						this.controller.danStats.maxCombo = score.maxCombo
					}
					this.controller.danStats.gauge = score.gauge
					this.controller.danStats.songIndex++

					// Trigger next song transition
					this.controller.loadNextDanSong()
					this.musicFadeOut++
				} else {

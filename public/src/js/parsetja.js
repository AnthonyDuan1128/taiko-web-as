		...
		for (var lineNum = 0; lineNum < this.data.length; lineNum++) {
			...
			if (line.slice(0, 1) === "#") {
				...
				if (line.indexOf(":") > 0) {
					...
					} else if (name.match(/^exam[1-4]$/)) {
						// EXAM1:type,red_pass,gold_pass,scope (for Dan-i Dojo)
						var examParts = value.split(",")
						if (examParts.length >= 3) {
							if (!currentCourse.exams) {
								currentCourse.exams = []
							}
							currentCourse.exams.push({
								id: parseInt(name.slice(-1)),
								type: examParts[0].trim().toLowerCase(),
								redPass: parseInt(examParts[1]) || 0,
								goldPass: parseInt(examParts[2]) || 0,
								scope: examParts[3] ? examParts[3].trim().toLowerCase() : "l"
							})
						}
						continue
					}
			...
			if (line.slice(0, 1) === "#") {
				...
				switch (name) {
					...
					case "lyric":
						...
						break
					// Dan-i Dojo: #NEXTSONG title,artist,genre,wave,offset,scoreInit
					default:
						if (name.startsWith("nextsong")) {
							var songParts = value.split(",")
							if (songParts.length >= 4) {
								...
								this.nextSongs.push({
									ms: ms,
									title: songParts[0].trim(),
									artist: songParts[1].trim(),
									genre: songParts[2].trim(),
									wave: songParts[3].trim(),
									offset: songParts[4] ? parseFloat(songParts[4]) : 0,
									scoreInit: songParts[5] ? parseInt(songParts[5]) : 0
								})
							}
						}
						break
